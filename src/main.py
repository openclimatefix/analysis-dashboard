...
if check_password():
    ...
    st.markdown(f'<h1 style="color:#63BCAF;font-size:48px;">{"Nowcasting Forecast Statistics"}</h1>',
    ...
    fig = px.bar(df_mae, x="datetime_utc", y="MAE", title='MAE Nowcasting', hover_data=['MAE'],
                 color_discrete_sequence=['#FFAC5F'], )
    st.plotly_chart(fig, theme="streamlit")

    fig2 = px.line(df_mae, x="datetime_utc", y="MAE", title='MAE Nowcasting Forecast', hover_data=['MAE'],
    ...
    with connection.get_session() as session:
        ...
        for forecast_horizon in forecast_horizon_selection:
            ...
            fig2.add_traces(
                [go.Scatter(x=df["datetime_utc"], y=df["MAE"], name=f"{forecast_horizon}-minute horizon", )])

        st.plotly_chart(fig2, theme="streamlit")

    # comparing MAE and RMSE
    fig3 = px.line(df_mae, x="datetime_utc", y="MAE", title='Nowcasting MAE with RMSE for Comparison',
                   hover_data=['MAE'],
                   color_discrete_sequence=['#FFD053', '#FFAC5F', '#ff9736', "#7BCDF3", "#086788", "#63BCAF",
                                            "#4C9A8E"], )
